...
# here is my implementation Ethereum ecdsa signature :)
def ecdsa_raw_sign_with_random_k(msghash, priv):

    z = bytes_to_int(msghash)
    seed = sha3(randint(0, 2**256))
    k = deterministic_generate_k(msghash, seed)
    print("k: ", k)
    r, y = multiply(G, k)
    s = inv(k, N) * (z + r * bytes_to_int(priv)) % N
